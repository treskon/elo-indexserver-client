...
@_attrs_define
class UserProfileC:
    """The constants in this class exist due to compatibility requirements with older Index Server versions, which gave
    back
     ...
     Organisation: ELO Digital Office GmbH
        ...
        Attributes:
            ...
            key_extend_document_access_in_workflow (Union[Unset, str]): Extended access control for documents or folders
                used in workflows.
                <p>
                 There are two concepts of extending access control for folders and documents routed though a workflow. The
                rules
                 are only applied if (a) the current user has an active workflow task for the folder or document and (b) the
                right
                 AccessC.FLAG2_EXTEND_WORKFLOW_RIGHTS is assigned to the current user.
                 </p>
                 <p>
                 1. Overlay additional access rights over the objects ACL: the lower 8 bits of the keys value specify a
                combination
                 of AccessC.LUR_* constants that are added to the ACL access rights when access checking is performed. The
                objects
            ...
            key_password_rules (Union[Unset, str]): Password rules. The password rules are stored as a comma separated list
                of 5 Integers, e.g. "7,1,1,0,0". 1.
                Minimum
                 password length 2. Is != 0, if password must contain at least one letter. 3. Is != 0, if password must contain
                at
